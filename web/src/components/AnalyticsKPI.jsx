...
export default function AnalyticsKPI({ analytics = {}, clockMinutes = 0, fleet = [] }) {
  ...
  const activeUnits = fleet.filter(v => v.state !== 'IDLE_STATION').length;
  const utilizationPct = fleet.length > 0 ? (activeUnits / fleet.length) * 100 : 0;
  ...
  const getEventBadge = (type) => {
    switch (type) {
      ...
      default:
        return <span className="badge font-mono text-[9px]">{type}</span>;
    }
  };
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="glass-panel p-3 flex flex-col gap-2">
        ...
        <div className="overflow-y-auto max-h-32 flex flex-col gap-1 pr-1 font-mono text-xs">
