...
export default function IncidentQueue({ 
  ...
  const handleSubmit = (e) => {
    ...
    const parsedX = parseFloat(coordX);
    const parsedY = parseFloat(coordY);

    if (onCreateIncident) {
      onCreateIncident({
        type: type || 'FIRE',
        severity: isNaN(parsedSev) ? 3 : Math.max(1, Math.min(5, parsedSev)),
        x: isNaN(parsedX) ? 5.0 : Math.max(0.5, Math.min(24.5, parsedX)),
        y: isNaN(parsedY) ? 5.0 : Math.max(0.5, Math.min(24.5, parsedY)),
        description: (description || '').trim() || `${type || 'FIRE'} Call Level ${severity}`
      });
    }
  };
    ...
    <div className="glass-panel p-4 flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-mono uppercase tracking-wider text-slate-400">
            Incident Log ({safeIncidents.length} Calls Total)
          </span>
        ...
        <div className="overflow-y-auto max-h-48 flex flex-col gap-1.5 pr-1">
          {safeIncidents.map((inc) => {
            if (!inc) return null;
            const incSev = typeof inc.severity === 'number' ? inc.severity : (parseInt(inc.severity, 10) || 1);
            const offRoad = inc.offRoadDistanceKm != null ? Number(inc.offRoadDistanceKm) : 0;
            const effPrio = inc.effectivePriority != null ? Number(inc.effectivePriority) : incSev;
            const assignedList = Array.isArray(inc.assignedVehicles) ? inc.assignedVehicles :
                                 Array.isArray(inc.assignedVehicleIds) ? inc.assignedVehicleIds :
                                 Array.isArray(inc.assignedUnits) ? inc.assignedUnits : [];

            return (
              <div 
                key={inc.id || Math.random().toString()}
              ...
              >
                <div className="flex items-center gap-2">
                  <span className={`px-1.5 py-0.5 rounded font-mono font-bold text-[10px] ${
                    incSev >= 4 ? 'bg-rose-950 text-rose-300 border border-rose-800' : 'bg-amber-950 text-amber-300 border border-amber-800'
                  }`}>
                    L{incSev}
                  </span>
                  <div>
                    <div className="font-bold text-slate-200 flex items-center gap-1.5">
                      <span>{inc.id || 'INC'}</span>
                      <span className="text-[10px] text-slate-400">({inc.type || 'UNKNOWN'})</span>
                      {offRoad > 0.1 && (
                        <span className="text-[9px] font-mono text-orange-400 bg-orange-950/60 px-1 rounded border border-orange-800/60">
                          +{offRoad.toFixed(1)}km off-road
                        </span>
                      )}
                      {inc.isEscalated ? (
                        <span className="text-[9px] font-mono px-1.5 py-0.5 rounded font-bold bg-amber-950 text-amber-300 border border-amber-600 animate-pulse">
                          STARVATION PROTECTED (L{effPrio.toFixed(1)})
                        </span>
                      ) : (inc.status === 'PENDING' || inc.status === 'PREEMPTED_QUEUED') && effPrio > incSev ? (
                        <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-sky-950/80 text-sky-300 border border-sky-800/80">
                          Escalating: L{incSev} ➔ L{effPrio.toFixed(1)} (+0.25/m)
                        </span>
                      ) : null}
                    </div>
                    <div className="text-[11px] text-slate-400">{inc.description || 'Emergency Incident'}</div>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-end gap-1.5">
                  <div className="flex items-center gap-1.5">
                    ...
                    {inc.status !== 'RESOLVED' && onResolveIncident && inc.id && (
                      <button
